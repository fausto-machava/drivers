$(document).ready(function(){
    var tabela = $('#tabelaModelo').DataTable();
    tabela.on('click','.btnEditar', function(){
        ...
        $('#txtNome').val(dados[1]);
        $('#jcFabricantes').val(dados[2]);
        $('#txtDescricao').val(dados[3]);
        $('#formUpdModelo').attr('action', '/editModelo/'+dados[0])
        $('#edit').modal('show');
    })
    ...
    tabela.on('click','.btnEliminar', function(){
        ...
        $('#formDelModelo').attr('action', '/delModelo/'+dados[0])
        $('#deletar').modal('show');
    })
